...
@Component
public class SyncIgdbGameExpansionsProcessor {
    private static final Logger logger = Logger.getLogger(SyncIgdbGameExpansionsProcessor.class.getName());
    ...
    public SyncIgdbGameExpansionsProcessor(MongoTemplate mongoTemplate) {
    ...
    @PostConstruct
    public void init() {
        logger.info("SyncIgdbGameExpansionsProcessor initialized");
        ...
        while (true) {
            String igdbQuery = String.format("""
                ...
                videos.name,videos.video_id,game_status,game_type;
                    where updated_at > %d & version_parent = null & game_type = (0,8,9);
                    sort id asc;
                    limit %d;
                    offset %d;
