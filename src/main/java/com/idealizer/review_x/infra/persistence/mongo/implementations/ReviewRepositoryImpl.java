...
public class ReviewRepositoryImpl implements ReviewRepositoryCustom {
        ...
        public Page<BaseReview> searchProjected(String username, FindReviewDTO f, Pageable pageable) {
                ...
                if (f.ratingMin() != null || f.ratingMax() != null) {
                        ...
                        if (f.ratingMin() != null)
                                rc = rc.gte(f.ratingMin());
                        if (f.ratingMax() != null)
                                rc = rc.lte(f.ratingMax());
                ...
                if ("rating".equals(sortDbField)) {
                        ...
                        sortAndPage.add(Aggregation
                                        .sort(Sort.by(dir, "ratingSort").and(Sort.by(Sort.Direction.DESC, "_id"))));
                } else {
                        sortAndPage.add(Aggregation
                                        .sort(Sort.by(dir, sortDbField).and(Sort.by(Sort.Direction.DESC, "_id"))));
                }
                ...
                sortAndPage.add(new RawAggregationOperation(new Document("$project",
                                new Document()
                                                ...
                                                .append("targetSlug",
                                                                new Document("$ifNull",
                                                                                Arrays.asList("$targetSlug",
                                                                                                "$target_slug")))
                                                .append("targetCover",
                                                                new Document("$ifNull",
                                                                                Arrays.asList("$targetCover",
                                                                                                "$target_cover")))
                                                .append("targetType",
                                                                new Document("$ifNull",
                                                ...
                                                .append("updatedAt",
                                                                new Document("$ifNull",
                                                                                Arrays.asList("$updatedAt",
                                                                                                "$updated_at")))
                                                .append("startedAt",
                                                                new Document("$ifNull",
                                                                                Arrays.asList("$startedAt",
                                                                                                "$started_at")))
                                                .append("finishedAt",
                                                                new Document("$ifNull",
                                                                                Arrays.asList("$finishedAt",
                                                                                                "$finished_at")))
        ...
        public Page<BaseTimelineItem> timeline(String username, FindTimelineDTO f, Pageable pageable) {
                ...
                Document rated = new Document("$cond", Arrays.asList(
                                ...
                                new Document("type", "RATED")
                                                .append("at", new Document("$ifNull",
                                                                Arrays.asList("$pg.updated_at", "$created_at"))),
                                null));
                ...
                Aggregation dataAgg = Aggregation
                                .newAggregation(Stream.concat(base.stream(), dataTail.stream()).toList());
